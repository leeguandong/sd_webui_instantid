...
class StableDiffusionXLInstantIDPipeline(StableDiffusionXLControlNetPipeline):
    ...
    def __call__(
        ...
        if control_mask is not None:
            ...
            region_mask = torch.from_numpy(np.array(control_mask)[:, :, 0]).to(self.unet.device,
                                                                               dtype=self.unet.dtype) / 255.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(self.controlnet, MultiControlNetModel):
                    ...
                    for control_index in range(len(self.controlnet.nets)):
                        ...
                        if control_index == 0 and control_mask_wight_image_list is not None:
                            down_block_res_samples = [
                                ...
                                for down_block_res_sample, mask_weight in
                                zip(down_block_res_samples, control_mask_wight_image_list)
                            ]
                    ...
                    down_block_res_samples = [torch.stack(down_block_res_samples).sum(dim=0) for down_block_res_samples
                                              in
