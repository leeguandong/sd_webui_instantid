...
from instantid.model_util import get_torch_device, load_models_xl
from instantid.pipeline_stable_diffusion_xl_instantid_full import StableDiffusionXLInstantIDPipeline
...
class InstantIDInterface:
    def __init__(self, pretrained_model_name_or_path="wangqixun/YamerMIX_v8"):
        self.pipe = None
        self.init_sdxl(pretrained_model_name_or_path)

    def init_sdxl(self, pretrained_model_name_or_path="wangqixun/YamerMIX_v8"):
    ...
    def draw_kps(self, image_pil, kps,
    ...
    def generate_image(self,
        ...
        face_kps = self.draw_kps(self.convert_from_cv2_to_image(face_image_cv2), face_info['kps'])

        if pose_image_path is not None:
            pose_image = load_image(pose_image_path)
            pose_image = self.resize_img(pose_image)
            ...
            face_kps = self.draw_kps(pose_image, face_info['kps'])
...
interface = InstantIDInterface()


def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as InstantID:
        with gr.Row():
            with gr.Column():
                # upload face image
                ...
                enable_LCM = gr.Checkbox(
                    label="Enable Fast Inference with LCM", value=True,
            ...
            submit.click(
                fn=interface.randomize_seed_fn,
            ...
            enable_LCM.input(fn=interface.toggle_lcm_ui, inputs=[enable_LCM], outputs=[num_steps, guidance_scale],
                             queue=False)

    return [(InstantID, "InstantID", "InstantID")]
